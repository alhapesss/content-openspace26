  ...
  return (
    <div className="flex flex-wrap gap-2.5 my-4">
      {STATUSES.map((s) => {
        ...
        return (
          <div key={s} className="bg-[#262622] border border-[#4d4d47] rounded-sm px-4 py-2.5 min-w-[90px]">
            <div className="text-[22px] font-extrabold">{n}</div>
            <div className="font-mono text-[10px] uppercase tracking-wider text-[#b8b8b0]">{s}</div>
          </div>
        )
      })}
      <div className="bg-[#262622] border border-[#4d4d47] rounded-sm px-4 py-2.5 flex-1 min-w-[180px]">
        <div className="text-[22px] font-extrabold">{pct}%</div>
        <div className="font-mono text-[10px] uppercase tracking-wider text-[#b8b8b0]">
          Progress Publish ({published}/{total})
        </div>
        <div className="h-1.5 bg-[#2e2e2a] rounded-full overflow-hidden mt-2">
          <div
